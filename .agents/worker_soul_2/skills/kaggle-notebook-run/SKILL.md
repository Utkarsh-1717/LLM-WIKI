...
5. Write `kernel-metadata.json` — ensure slug matches title
6. `kaggle kernels push -p <dir>` — capture full stdout
7. **Parse real slug from push output URL**
8. **Parse real slug from push output URL**
9. Start `kaggle-pulse-check` skill using the real slug (v3.0.0+):
...
10. On COMPLETE: verify dataset published, report URL + file sizes to user
11. On ERROR (confirmed 3×): pulse-check auto-fetches log, parse last 80 lines, fix and re-push

...
KERNEL = "utkarshpatelthefirst/pairs-full-pipeline-v3"
print(f"🔍 Monitoring: {KERNEL}", flush=True)

def check_status(kernel_ref):
    try:
        result = subprocess.run(
            ['bash', '-c', f'export $(cat ~/.quant_env | xargs) && kaggle kernels status {kernel_ref}'],
...
def fetch_error_log(kernel_ref):
    ...
    subprocess.run(
        ['bash', '-c', f'export $(cat ~/.quant_env | xargs) && kaggle kernels output {kernel_ref} -p {log_dir} --force'],
